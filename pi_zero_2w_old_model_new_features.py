...
def prepare_audio(path, norm):
    """
    NEW FEATURE PROCESSING + OLD TFLITE MODEL

    Audio processing is intentionally changed to the lightweight
    python_speech_features pipeline used for Raspberry Pi:
      - mono
      - resample to 16 kHz with scipy
      - DC removal
      - peak normalization
      - 24 MFCC
      - 25 ms window
      ...
      - nfft = 1024
      - fixed 500 frames
      - external normalization from normalization.json, if compatible

    IMPORTANT:
    The neural-network model is still the OLD model:
        models/baby_audio_strong.tflite

    This is a feature-extractor/model mismatch if the old model was trained
    with librosa MFCCs. The code is useful for testing the combination, but
    accuracy must be measured on the held-out test set.
    """
    sr_target = int(norm.get('sample_rate', 16000))
    n_mfcc = int(norm.get('n_mfcc', 24))
    max_frames = int(norm.get('max_frames', 500))

    # New Pi-style MFCC settings.
    WINLEN = 0.025
    WINSTEP = 0.010
    NFFT = 1024

    # Use normalization.json only when it has exactly one value per MFCC.
    mean = np.asarray(norm.get('mean', np.zeros(n_mfcc)), dtype=np.float32).reshape(-1)
    std = np.asarray(norm.get('std', np.ones(n_mfcc)), dtype=np.float32).reshape(-1)

    if len(mean) != n_mfcc or len(std) != n_mfcc:
        print('WARNING: normalization.json mean/std size does not match n_mfcc.')
        print('Using zero mean / unit std instead.')
    ...
    # Exact lightweight resampling path used for Pi-style processing.
    ...
    # Same basic signal conditioning used by the new Pi feature pipeline.
    audio -= np.mean(audio)
    ...
    # python_speech_features instead of librosa.feature.mfcc()
    features = mfcc(
        audio,
    ...
    if original_frames < max_frames:
        features = np.pad(
            features,
            ((0, max_frames - original_frames), (0, 0)),
            mode='constant'
        )
        action = f'PAD {max_frames - original_frames} frames'
    else:
        features = features[:max_frames, :]
        action = f'TRUNCATE to {max_frames} frames' if original_frames > max_frames else 'NO padding/truncation'

    # Keep the OLD model's expected external normalization.
    features = ((features - mean) / std).astype(np.float32)

    # Old model input: (1, 500, 24, 1)
    ...
    return features, {
        ...
        'feature_method': 'python_speech_features.mfcc',
        'winlen': WINLEN,
        'winstep': WINSTEP,
        'nfft': NFFT,
        'n_mfcc': n_mfcc
...
def main():
    limit_cpu()
    print('\n' + '=' * 70)
    print('\nRASPBERRY PI ZERO 2 W - LIMITED PC TEST')
    print('OLD MODEL + NEW FEATURES')
    print('=' * 70)
    print('GPU                  : DISABLED')
    print('Maximum CPU cores    : 4')
    print('TFLite threads       : 4')
    print('Batch size           : 1')
    print('MFCC                 : NEW PSF FEATURE PROCESSING')
    ...
    while True:
        ...
        try:
            ...
            print('Feature method       :', info['feature_method'])
            print('MFCC settings        :', f"{info['n_mfcc']} MFCC, {info['winlen']*1000:.0f} ms, {info['winstep']*1000:.0f} ms, nfft={info['nfft']}")
